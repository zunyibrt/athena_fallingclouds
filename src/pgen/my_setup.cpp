  ...
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      for (int i=is; i<=ie; i++) {
        ...
        rd = (P_bg/T_bg)*std::exp(-x3/scale_height);
        rvx = 0.0;
        rvy = 0.0;
        rvz = 0.0;
        rp = P_bg*std::exp(-x3/scale_height);
...
void Mesh::UserWorkInLoop() { 
  ...
  for (int bn=0; bn<nblocal; ++bn) {
    ...
    for (int i=pmb->is; i<=pmb->ie; ++i) {
      ...
      Real temp = press/u_d;
      //Real eint = phyd->u(IEN,k,j,i)
      //              - 0.5 *(phyd->u(IM1,k,j,i)*phyd->u(IM1,k,j,i)
      //                    + phyd->u(IM2,k,j,i)*phyd->u(IM2,k,j,i)
      //                    + phyd->u(IM3,k,j,i)*phyd->u(IM3,k,j,i))/u_d;
      //Real temp = eint * (pmb->peos->GetGamma()-1.0)/u_d;
  ...
  for (int k=1; k<=ngh; k++) {
    for (int j=jl; j<=ju; j++) {
      for (int i=il; i<=iu; i++) {
        ...
        prim(IDN,kl-k,j,i) = std::exp(-(x3+box_height)/scale_height);
        prim(IPR,kl-k,j,i) = std::exp(-(x3+box_height)/scale_height);
  ...
  for (int k=1; k<=ngh; k++) {
    for (int j=jl; j<=ju; j++) {
      for (int i=il; i<=iu; i++) {
        ...
        prim(IDN,ku+k,j,i) = std::exp(-(x3+box_height)/scale_height);
        prim(IPR,ku+k,j,i) = std::exp(-(x3+box_height)/scale_height);
